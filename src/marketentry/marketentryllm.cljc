(ns marketentry.marketentryllm
  ...
  the IRQ public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real government-contract
  ...
  engagement's own `:cited-authority` matches
  `marketentry.facts/expected-investment-authority` for its
  `:governorate` -- a mismatch is a genuine market-entry error (the
  wrong investment authority for the Kurdistan Region vs. federal
  Iraq), not a stylistic slip, so a low-confidence proposal here still
  gets independently re-checked (and rejected) by
  `marketentry.governor`'s flagship `krg-authority-mismatch` check
  regardless of what confidence this advisor reports.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world government-contract filing
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)
        expected-authority (when e (facts/expected-investment-authority
                                     (:jurisdiction e) (:governorate e)))
        authority-ok? (or (nil? expected-authority)
                          (= expected-authority (:cited-authority e)))]
    {:summary    (str subject " 向け政府調達フィリング提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "governorate=" (:governorate e)
                        " cited-authority=" (:cited-authority e)
