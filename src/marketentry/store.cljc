(ns marketentry.store
  "SSoT for the IRQ market-entry compliance actor, behind a `Store`
  ...
  `:governorate` + `:cited-authority` are IRQ-specific fields the
  flagship governor check (`marketentry.governor`'s
  `krg-authority-mismatch`) reads: Iraq structurally has TWO
  investment-licensing regimes (federal NIC vs. Kurdistan Board of
  Investment), so an engagement records BOTH which governorate it
  actually operates in AND which authority the advisor's proposal
  cited, so the governor can independently cross-check them.
...
(defn demo-data
  ...
  `eng-4` is the flagship scenario: a Kurdistan Region (Erbil)
  engagement whose proposal wrongly cites the FEDERAL authority
  (`:federal`) instead of `:krg-boi`. `eng-6` is the matching POSITIVE
  case -- a Kurdistan Region engagement that correctly cites
  `:krg-boi` -- proving the check does not simply flag 'any mention of
  the Kurdistan Region', only an actual mismatch."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Dijla Systems Baghdad" :governorate "Baghdad"
             ...
             :jurisdiction "IRQ" :status :intake}
    ...
    "eng-3" {:id "eng-3" :operator "Furat Trading" :governorate "Basra"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "IRQ" :status :intake}
    "eng-4" {:id "eng-4" :operator "Hewler Export Co" :governorate "Erbil"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :cited-authority :federal
             :requires-cr? true :cr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "IRQ" :status :intake}
    "eng-5" {:id "eng-5" :operator "Ninawa Logistics" :governorate "Nineveh"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :cited-authority :federal
             ...
             :drafted? false :submitted? false
             :jurisdiction "IRQ" :status :intake}
    "eng-6" {:id "eng-6" :operator "Sulaymaniyah Civic Tech" :governorate "Sulaymaniyah"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :cited-authority :krg-boi
             :requires-cr? true :cr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "IRQ" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator governorate base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    cited-authority                       (assoc :engagement/cited-authority cited-authority)
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :governorate (:engagement/governorate m)
     ...
     :cited-authority (:engagement/cited-authority m)
