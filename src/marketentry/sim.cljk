(ns marketentry.sim
  ...
  KRG/federal investment-authority mismatch."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (IRQ, Baghdad, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (Erbil, sets up krg-authority-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (cited :federal but Erbil is KRG BOI territory -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up cr-unverified) ==")
    ...
    (println "== filing/submit eng-5 (cr-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (Sulaymaniyah, correctly cites krg-boi) ==")
    ...
    (println "== filing/submit eng-6 (krg-boi correctly cited for Sulaymaniyah -> escalates then commits) ==")
