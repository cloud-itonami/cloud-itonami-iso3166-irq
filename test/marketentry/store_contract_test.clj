...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "IRQ"
                                   :governorate "Baghdad" :cited-authority :federal
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "IRQ" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:governorate (:engagement m)) (:governorate (:engagement d))))
    (is (= (:cited-authority (:engagement m)) (:cited-authority (:engagement d))))
