  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Dijla Systems Baghdad"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Dijla Systems Baghdad" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest krg-authority-mismatch-is-held-and-unoverridable
  (testing "Erbil engagement citing the federal authority (instead of KRG BOI) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:krg-authority-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest krg-authority-correctly-cited-is-not-flagged
  (testing "Sulaymaniyah engagement correctly citing :krg-boi -> no krg-authority-mismatch violation"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-6")
          _ (draft! actor "t5bpre" "eng-6")
          r1 (exec-op actor "t5b" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :interrupted (:status r1)) "still escalates for human approval (actuation), but NOT a HOLD")
...
(deftest cr-unverified-is-held-and-unoverridable
  (testing "unverified Companies Registration Directorate number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:cr-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
